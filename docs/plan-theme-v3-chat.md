...
**Two unread-dot keys were added after review.** The dot had no key of its own
and borrowed `--primary-button-text`; on a theme with dark button text
(drurple's is `#1b0436`) that left it invisible against the rail. There is one
